~function () {
   /* var effect = { //这一组对象里都是存放如何去计算当前位置的公式
        linear:function (t,b,c,d){//定义一个匀速运动效果
            return b + (t/d)*c;
        }
    }*/
    var zhufengEffect = {
     ...
     * @param target Objecct需要运动的终点
     * @param duration 需要运动的时间间隔
     */
    function move(curEle, target, duration,effect,callback) { //驱动元素运动的 //target{width:100,left:50}
        ...
        for(var key in target){ //我们需要通过target的维度（属性多少）,来查找我们begin的属性，如果不是target的属性，我们这个维度就没有必要获取了。
            if(target.hasOwnProperty(key)){
                ...
                begin[key] = DOM.getCss(curEle,key);//begin{width:100,left:0}
                change[key] = target[key] - begin[key]; //change:{width:差,left:差}
            }
        }
...
duration); //获取单个维度的值
                    DOM.setCss(curEle,k,curPosi/*维度*//*维度的值*/);
                }
            }
        },interval);
    }
